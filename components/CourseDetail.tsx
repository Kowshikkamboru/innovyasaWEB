...
import {
  ...
  Globe, Subtitles, FileText, Award, Heart, Share2, Lock, Infinity as InfinityIcon, ChevronLeft
} from "lucide-react";
import { FadeIn } from "@/components/AnimationUtils";

// Shared sample course used as default when no prop provided
const SAMPLE_COURSE = {
...
export const DEFAULT_COURSE = SAMPLE_COURSE;

export default function CourseDetail({ course = SAMPLE_COURSE }: { course?: any }) {
    ...
    <div className="overflow-x-hidden bg-white">
      ...
      <section className="bg-gradient-to-br from-navy via-[#1A2E5A] to-navy relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 py-14 relative">
          <div className="grid lg:grid-cols-[1fr_380px] gap-12 items-start">
            <div>
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-wrap gap-2 mb-5">
                <span className="badge-orange">{course.category}</span>
                <span className="bg-white/10 text-gray-300 text-xs font-semibold px-3 py-1 rounded-full border border-white/10">{course.level}</span>
                {course.tags.map((t:any) => (
                  <span key={t} className="text-[11px] bg-white/5 text-gray-400 px-2.5 py-0.5 rounded-full border border-white/10">{t}</span>
                ))}
              </motion.div>

              <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-3xl md:text-4xl lg:text-5xl font-bold text-white leading-tight mb-4" style={{ fontFamily: "Sora, sans-serif" }}>
                {course.title}
              </motion.h1>

              <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="text-gray-300 text-lg mb-6">
                {course.subtitle}
              </motion.p>

              {/* Rating */}
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }} className="flex flex-wrap items-center gap-4 mb-6">
              ...
              {/* Meta */}
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }} className="flex flex-wrap gap-5 text-sm text-gray-400">
                {[
            ...
            <motion.div ref={sidebarRef} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="hidden lg:block">
              <div className="bg-white rounded-3xl overflow-hidden shadow-2xl">
                <div className={`h-52 bg-gradient-to-br ${course.thumbnailGradient} flex items-center justify-center relative`}>
                  ...
                  <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }} className="absolute inset-0 flex items-center justify-center bg-black/20">
                    <div className="w-16 h-16 bg-white/90 rounded-full flex items-center justify-center shadow-xl">
                      <Play className="w-7 h-7 text-navy fill-navy ml-1" />
                    </div>
                  </motion.button>
                  <div className="absolute top-3 right-3 bg-primary text-white text-xs font-bold px-2.5 py-1 rounded-full">{discount}% OFF</div>
                </div>

                <div className="p-6">
                  <div className="flex items-center gap-3 mb-4">
                    <span className="text-3xl font-black text-navy" style={{ fontFamily: "Sora, sans-serif" }}>₹{course.price.toLocaleString("en-IN")}</span>
                    <span className="text-gray-400 text-base line-through">₹{course.originalPrice.toLocaleString("en-IN")}</span>
                  </div>

                  <Link href="#enroll" className="btn-primary w-full justify-center py-4 text-base mb-3">Enroll Now</Link>
                  ...
                  <div className="flex gap-2 mt-5 pt-5 border-t border-gray-100">
                    <button onClick={() => setWishlisted(!wishlisted)} className={`flex-1 flex items-center justify-center gap-2 text-xs font-semibold py-2.5 rounded-lg border transition-all ${wishlisted ? "border-rose-200 text-rose-500 bg-rose-50" : "border-gray-200 text-gray-500 hover:border-gray-300"}`}>
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex gap-1 overflow-x-auto">
            {(["overview", "curriculum", "instructor", "reviews"] as const).map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab)} className={`relative px-5 py-4 text-sm font-semibold capitalize whitespace-nowrap transition-colors ${activeTab === tab ? "text-primary" : "text-gray-500 hover:text-gray-800"}`}>
                {tab}
                {activeTab === tab && (
      ...
      <div className="max-w-7xl mx-auto px-4 py-12">
        <div className="grid lg:grid-cols-[1fr_380px] gap-14">
            ...
            <AnimatePresence mode="wait">
              {activeTab === "overview" && (
                <motion.div key="overview" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                  <section className="mb-12">
                    <h2 className="text-2xl font-bold text-navy mb-6" style={{ fontFamily: "Sora, sans-serif" }}>What You&apos;ll Learn</h2>
                    <div className="border border-gray-100 rounded-2xl p-6 bg-gray-50/50">
                      <div className="grid sm:grid-cols-2 gap-3">
                        {course.whatYouLearn.map((item:any) => (
                  ...
                  <div className="grid sm:grid-cols-2 gap-6 mb-12">
                    {[{ title: "Assignment", desc: "Practical design challenges after each module. Submit via Figma link and receive mentor feedback within 48 hours.", icon: "📋" },{ title: "Prerequisites", desc: "No prior design experience needed. A computer with internet access and willingness to learn is all you need.", icon: "🎯" }].map(item => (
                  ...
                  <section className="mb-12">
                    <h2 className="text-2xl font-bold text-navy mb-6" style={{ fontFamily: "Sora, sans-serif" }}>Description</h2>
                  ...
                  <section>
                    <h2 className="text-2xl font-bold text-navy mb-6" style={{ fontFamily: "Sora, sans-serif" }}>Requirements</h2>
              ...
              {activeTab === "curriculum" && (
                <motion.div key="curriculum" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                  <div className="flex items-center justify-between mb-6">
                    <h2 className="text-2xl font-bold text-navy" style={{ fontFamily: "Sora, sans-serif" }}>Course Content</h2>
                  ...
                  <div className="space-y-3">
                    {course.curriculum.map((section:any, si:number) => (
                      <div key={section.title} className="border border-gray-100 rounded-2xl overflow-hidden">
                        <button onClick={() => setOpenSection(openSection === si ? null : si)} className="w-full flex items-center justify-between px-6 py-4 bg-gray-50 hover:bg-gray-100 transition-colors text-left">
                          ...
                          <span className="text-xs text-gray-500 flex-shrink-0 ml-4">{section.lectures} lectures • {section.duration}</span>
                        </button>
                                  ...
                                  <div key={item.title} className="flex items-center justify-between px-6 py-3.5 hover:bg-primary/2 transition-colors">
                                    <div className="flex items-center gap-3 min-w-0">
                                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${item.free ? "bg-primary/10" : "bg-gray-100"}`}>
                                        {item.free ? <Play className="w-3.5 h-3.5 text-primary" /> : <Lock className="w-3.5 h-3.5 text-gray-400" />}
                                      </div>
                                      <span className={`text-sm truncate ${item.free ? "text-primary font-medium" : "text-gray-700"}`}>{item.title}{item.free && <span className="ml-2 text-[10px] bg-primary/10 text-primary px-1.5 py-0.5 rounded-full font-semibold">FREE</span>}</span>
                                    </div>
                                    <span className="text-xs text-gray-400 flex-shrink-0 ml-4 flex items-center gap-1"><Clock className="w-3 h-3" /> {item.duration}</span>
                                  </div>
                                ))}
                              </div>
                            </motion.div>
              ...
              {activeTab === "instructor" && (
                <motion.div key="instructor" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                  <h2 className="text-2xl font-bold text-navy mb-8" style={{ fontFamily: "Sora, sans-serif" }}>Your Instructor</h2>
                  <div className="flex items-start gap-6 mb-8">
                    <div className="w-20 h-20 bg-gradient-to-br from-purple-500 to-violet-600 rounded-2xl flex items-center justify-center text-white text-2xl font-bold flex-shrink-0">{course.instructor.avatar}</div>
                    <div>
              ...
              {activeTab === "reviews" && (
                <motion.div key="reviews" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                  <h2 className="text-2xl font-bold text-navy mb-8" style={{ fontFamily: "Sora, sans-serif" }}>Student Reviews</h2>
                      ...
                      {[5, 4, 3, 2, 1].map(star => (
                        <div key={star} className="flex items-center gap-3 mb-2">
                          <div className="flex-1 h-2 bg-gray-200 rounded-full overflow-hidden">
                            <div className="h-full bg-amber-400 rounded-full" style={{ width: star === 4 ? "50%" : star === 5 ? "30%" : star === 3 ? "15%" : "5%" }} />
                          </div>
          ...
          {/* Right sticky sidebar for desktop */}
            ...
            <div className="sticky top-36">
              <div className="bg-white rounded-3xl border border-gray-100 shadow-xl overflow-hidden">
                <div className={`h-48 bg-gradient-to-br ${course.thumbnailGradient} flex items-center justify-center relative`}>
                  ...
                  <div className="absolute top-3 right-3 bg-primary text-white text-xs font-bold px-2 py-0.5 rounded-full">{discount}% OFF</div>
                </div>
                ...
                <div className="p-6">
                  ...
                  <Link href="/courses/enroll" className="btn-primary w-full justify-center py-4 mb-3 text-base">Enroll Now</Link>
                  ...
                  <div className="flex gap-2 mt-5 pt-5 border-t border-gray-100">
                    <button onClick={() => setWishlisted(!wishlisted)} className={`flex-1 flex items-center justify-center gap-1.5 text-xs font-semibold py-2.5 rounded-lg border transition-all ${wishlisted ? "border-rose-200 text-rose-500 bg-rose-50" : "border-gray-200 text-gray-500"}`}>
      ...
      <section id="enroll" className="bg-gradient-to-br from-primary to-orange-400 py-16 px-4">
        <div className="max-w-4xl mx-auto text-center">
          <FadeIn>
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4" style={{ fontFamily: "Sora, sans-serif" }}>Ready to Master UI/UX Design?</h2>
            <p className="text-white/80 mb-8">Join {course.enrolled.toLocaleString()} students already learning. Get lifetime access for just <strong>₹{course.price.toLocaleString("en-IN")}</strong>.</p>
            <div className="flex flex-wrap gap-4 justify-center">
              <Link href="/auth/signup" className="bg-white text-primary font-bold px-8 py-4 rounded-xl hover:bg-gray-50 transition-colors">Enroll Now — ₹{course.price.toLocaleString("en-IN")}</Link>
              <Link href="/courses" className="border-2 border-white text-white font-bold px-8 py-4 rounded-xl hover:bg-white/10 transition-colors flex items-center gap-2"><ChevronLeft className="w-4 h-4" /> Browse More Courses</Link>
            </div>
          </FadeIn>
        </div>
      </section>
